...
function buildBoard(jobs, events, tasks) {
  ...
  // 1) Drop exact duplicates (same task id), just in case.
  ...
  // 2) Collapse content-identical top-level tasks. The team shares one task
  // board across multiple logins, so the SAME to-do comes back once per member
  // as a separate task (different id, identical title + subtasks). We key on the
  // normalized title plus its sorted subtask titles, so each unique to-do shows
  // once while genuinely different to-dos are kept.
  const taskSignature = (t) => {
    const kids = (childrenByParent[t.id] || [])
      .map((s) => norm(s.text)).sort().join("|");
    return norm(t.text) + "||" + kids;
  };
  const seenSig = new Set();
  const topTasks = tasks.filter((t) => !t.parent).filter((t) => {
