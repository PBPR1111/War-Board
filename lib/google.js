...
async function fetchCalendarEvents({ days = 60 } = {}) {
  ...
  return (res.data.items || []).map((ev) => ({
    ...
    startDateTime: ev.start?.dateTime || null, // null = all-day event
    htmlLink: ev.htmlLink || "", // opens this exact event in Google Calendar
  }));
}
...
async function fetchTasks() {
  ...
  for (const list of lists) {
    ...
    for (const t of items) {
      out.push({
        id: t.id,
        parent: t.parent || null, // set on subtasks → id of the parent task
        text: t.title || "",
        notes: t.notes || "",
