...
async function leapGetAll(path, params = {}) {
  ...
  while (page <= MAX_PAGES) {
    ...
    try {
      ...
      // Some endpoints (e.g. /customers) reject a large per_page and error out.
      // Drop it and retry this page with the API's default page size so the
      // fetch still completes (just in more calls) instead of failing entirely.
